...
class NetworkHelper {
  ...
  /// update user
  Future<Map> updateUser(XFile? image, XFile? cover, Map<String, String> updates) async {
    try {
      // String firebaseToken = await FirebaseMessaging.instance.getToken() ?? '';
  ...
  Future<Map> applyForVerification(FilePickerResult document) async {
    try {
      ...
      if (document.paths.isNotEmpty) {
        request.files.add(await MultipartFile.fromPath("document", document.paths[0] ?? ''));
      }
      request.fields.addAll({
        'api_token': apiToken,
      });
  ...
  Future<Map> createPost(
    ...
    try {
      ...
      if (pdfFile != null && pdfFile.paths.isNotEmpty) {
        request.files.add(await MultipartFile.fromPath("pdf", pdfFile.paths[0] ?? ''));
      }
      request.fields.addAll({
        'api_token': apiToken,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (!decodedResponse['error']) {
          ...
          final String nextPageURL = currentPage == lastPage ? '' : decodedPosts['next_page_url'];
  ...
  Future<Map> getVerificationStatus() async {
    try {
      ...
      if (response.statusCode == 200) {
        final decodedResponse = jsonDecode(response.body);
        if (!decodedResponse['error']) {
          VerificationStatusModel verificationStatus = VerificationStatusModel.fromJson(decodedResponse);
          return {
            'error': false,
